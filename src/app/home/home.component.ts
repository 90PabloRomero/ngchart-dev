...
export class HomeComponent implements OnInit, AfterViewInit {
  ...
  dropAndSelect(event: any) {
    ...
    setTimeout((_) => {
      ...
      if (length == 1) {
        ...
        this.treeNodeCurrent = this.treeOrg.treeModel.getNodeBy((item) => {
          console.log(cell.attributes);
          if (cell.attributes.tree_id == item.data.id) {
            console.log("tree id exist");
            const currentNode = this.treeOrg.treeModel.getNodeById(
              cell.attributes.tree_id
            );
      ...
      if (this.sheetSelected.ID != 0) {
        ...
        if (toSelectCell) {
          ...
          toSelectCell.attributes.attrs[".sibling"].visibility =
            toSelectCell.attributes.org_level == "0" ? "hidden" : "visible"; //Hide sibling circles for root node.
        }
  ...
  selectGraphNode($event: any, tree: any) {
    ...
    if (this.sheetSelected.ID != 0) {
      ...
      //Seleccionar el nodo que genera el evento
      let toSelectCell = _.find(this.paperView.graph.getElements(), (cell) => {
        return cell.attributes.tree_id == $event.attributes.tree_id;
      });
  ...
  onDeleteGraphNodeEvent($event) {
    ...
    console.log(cell);
    let treeNode = this.treeOrg.treeModel.getNodeBy(
      (node) => node.id == cell.tree_id
    );
    TREE_ACTIONS.ACTIVATE(this.treeOrg, treeNode, event);

    setTimeout(() => {
      this.deleteNodesConfirmedByUser(treeNode);
    }, 1000);
  }
  ...
  treeFocusNextNode() {
    ...
    setTimeout((_) => {
      this.paperView.getNextCellByCurrentSearch();
    }, 300);
  }
  ...
  treeFocusPreviousNode() {
    ...
    setTimeout((_) => {
      this.paperView.getPreviousCellByCurrentSearch();
    }, 300);
  }
    ...
    _.each(this.projectSheets, (sheet) => {
      ...
      _.each(sheetCurr.cells, (shCell) => {
        if (
          shCell &&
          shCell.attrs &&
          shCell.tree_id == cell.attributes.tree_id
        ) {
  ...
  changeDedicationRegime(position: any, tree: any) {
    // change position dedication regime
    if (this.sheetSelected.ID != 0) {
      let cell = _.find(this.paperView.graph.getElements(), (item) => {
        return item.attributes.tree_id == position.ID;
      });
      var treeNode = this.treeOrg.treeModel.getNodeBy(
        (nodeIn) => nodeIn.data.id == position.ID
      );
  ...
  mouseDownGraphNode(event: any) {
    if (event.action == "up") {
      this.circleAddNodeNorth(this.treeOrg, event);
    } else if (event.action == "down") {
      this.circleAddNodeSouth(this.treeOrg, event);
    } else if (event.action == "left") {
      this.circleAddNodeWest(this.treeOrg, event);
    } else if (event.action == "right") {
      this.circleAddNodeEast(this.treeOrg, event);
    }
  }

  graphNodeSelected(event: any) {
    // on graph(sheet) node selected
    this.treeNodeCurrent = this.treeOrg.treeModel.getNodeBy((item) => {
      return event.attributes.tree_id == item.data.id;
    });
    this.treeOrg.treeModel.expandAll();
    TREE_ACTIONS.ACTIVATE(this.treeOrg, this.treeNodeCurrent, event);
    if (this.treeNodeCurrent) {
      ...
      if (!this.treeNodeCurrent.isRoot) {
        this.positionCurrent.PositionInmediateSuperior =
          this.removeAandTfromName(this.treeNodeCurrent.parent.data.name);
      }
  ...
  onNavChange(changeEvent: NgbNavChangeEvent) {
    // when selecting sheet tab
    this.saveBeforeLeaving();
    this.loadSheetByID(changeEvent.nextId);
  }

  onAccordionChange(changeEvent: NgbPanelChangeEvent) {
  ...
  getProjects() {
    ...
    this.http.get<any>(urlApi + "/project").subscribe(
      (any) => {
        if (any) {
          ...
          if (!this.projects[0]) {
            ...
            if (this.projectToEdit) {
              this.projectSelected = this.findProjectByName(
                this.projectToEdit.ProjectName
              );
  ...
  getContacts0() {
    this.http.get<any>(urlApi + "/contact").subscribe(
      (any) => {
        if (any) {
          // this.contacts = any;
          console.log(any);
  ...
  getSheets(project: any) {
    //get all sheets by project
    this.sheetSelected = new Sheet();
    this.sheetSelected.SheetName = "Sheet";
    this.isSheetNodesTempView = false;
    this.http.get<any>(urlApi + "/project/" + project.ID + "/sheets").subscribe(
      (any) => {
        if (any) {
  ...
  deleteCurrentSheet() {
    ...
    if (this.sheetSelected.ID != 0) {
      this.http
        .delete<any>(urlApi + "/sheet/" + this.sheetSelected.ID)
        .subscribe(
          (any) => {
            if (any) {
  ...
  isEmpty(allData: Object) {
    if (Object.values(allData)[0].length == 0) return true;
    if (Object.values(Object.values(allData)[1]).length == 0) return true;
  }
  ...
  loadProject(project: any) {
    ...
    console.log(
      "Load Project: Project Selected: " + this.projectSelected.ProjectName
    );
    console.log(this.projectSelected);
    this.sheets = [];
    this.activeSheets = {};
    ...
    this.nodes = [];
    if (
      Object.values(Object.values(allData)[0]).length ||
      ...
      setTimeout(() => {
        ...
        console.log(
          "Load Project: let root = this.treeOrg.treeModel.getFirstRoot() "
        );
        console.log(root);
        if (root) {
          root.setActiveAndVisible();
        }
      }, 300);
    }
  ...
  async exportG() {
    //  remove some styles  before export graph
  ...
  async exportAndDownload() {
    ...
    let paperCurrentSizeW =
      this.paperView.paperElement.nativeElement.offsetWidth;
    let paperCurrentSizeH =
      this.paperView.paperElement.nativeElement.offsetHeight;
    ...
    const blob: any = await canvas.convertToBlob({
      type: "image/png",
      quality: 1,
    });
  ...
  drawSheetFunctionalRels() {
    // functional relationships only store on tree, they are draw on every load
    this.treeOrg.treeModel.doForAll((item) => {
      if (item.data.isfunctionalrel) {
        console.log("drawSheetFunctionalRels");
        console.log(item);
        if (
          item.data.functionalRelSourceId &&
          item.data.functionalRelTargetId
        ) {
          this.paperView.addFunctionalRel(
            item.data.functionalRelSourceId,
            item.data.functionalRelTargetId
          );
        }
      ...
      _.forEach(cells.cells, (cell) => {
        if (cell.attrs[".rank"]) {
          ...
          if (cellName == tempName) {
            ...
            if (cell.attrs[".card"])
              cell.attrs[".card"].strokeDasharray = "5,10";
  ...
  loadSheet(sheet: any) {
    ...
    if (this.sheetSelected.Attrs != "") {
      // set jointjs  paper(sheet)  dimentions on sheet load
      let dim = JSON.parse(this.sheetSelected.Attrs);
      if (dim.w && dim.h) {
        let width =
          this.paperDimensions.width > dim.w
            ? this.paperDimensions.width
            : dim.w;
        let heigth =
          this.paperDimensions.heigth > dim.h
            ? this.paperDimensions.heigth
            : dim.h;
        this.paperView.paper.setDimensions(width, heigth);
      }
    } else {
      console.log("true");
      this.sheetSelected.Attrs = JSON.stringify({
        w: this.paperDimensions.width,
        h: this.paperDimensions.heigth,
      });
    }
  ...
  resizeSheet() {
    ...
    this.sheetSelected.Attrs = JSON.stringify({
      w: this.paperView.graph.getBBox().width + 200,
      h: this.paperView.graph.getBBox().height + 200,
    });
  }
  ...
  toggleClasses(sheetId) {
    !this.classes[sheetId]
      ? (this.classes[sheetId] = "activeSheet-class")
      : (this.classes[sheetId] = "");
  }
  ...
  openAddNodeModal(event, inputFormTemplate) {
    // open add node  modal
    this.newNodeName = "";
    this.newNodeCode = "";
    if (!this.sheetSelected.ID) {
  ...
  openReporsWindow(event, inputFormTemplate) {
    // open reports modal
    event.preventDefault();
    this.modalWindow = this.modalService.open(inputFormTemplate, {
      ariaLabelledBy: "modal-basic-title",
  ...
  openSheetForm(event, inputFormTemplate, sheet: Sheet) {
    // open sheet form modal
  ...
  openSheetFormEdit(event, inputFormTemplate, sheet: Sheet) {
    // open sheet edit form modal
    if (!this.projectSelected.ID) {
      alert("No project Selected!");
      return;
    }
  ...
  openProjectFormCustom(
    event,
    inputFormTemplate,
    project: Project,
    projectSuccessTemplate
  ) {
  ...
  refreshSelectedSheetAfterDelete(sheet: Sheet) {
    //refresh selected sheet after tree deleting
    this.positionCurrent = new Position();
    if (!this.sheetSelected.ID) {
      return;
    }
  ...
  refreshSelectedSheetCB(sheet: any, cb: any) {
    this.isSheetNodesTempView = false;
    this.http.get<any>(urlApi + "/sheet/" + sheet.ID).subscribe(
      (any) => {
        if (any) {
  ...
  refreshSelectedSheet(sheet: any) {
  ...
  saveOffviewSheet(sheet: any) {
    // on tree change this function is called to save sheet sync
    if (sheet.ID) {
      //
      //console.log("saveOffviewSheet: on tree change this function is called to save sheet sync ")
      //console.log(sheet.Data)
      this.http.put<any>(urlApi + "/sheet/" + sheet.ID, sheet).subscribe(
        (any) => {
          if (any) {
          }
        },
  ...
  saveSheet(sheet: any, isNameUpdate?: boolean) {
    // save sheet on db
    if (sheet.ID) {
      //if exists
      if (!sheet.SheetName || sheet.SheetName == "") {
      ...
      //console.log(sheet.Data)
      this.http
        .put<any>(urlApi + "/sheet/" + sheet.ID + querystring, sheet)
        .subscribe(
          (any) => {
            if (any) {
              this.refreshSelectedSheetCB(sheet, (sh) => {
      ...
      this.http.post<any>(urlApi + "/sheet", sheet).subscribe(
        (any) => {
          if (any) {
            this.getSheets(this.projectSelected);
            setTimeout(() => {
  ...
  saveProject(project: any, isNameUpdate?: boolean) {
    // save  project on db
    if (
      this.projectEvent.includes("bi bi-pen") &&
    ...
    if (project.ID) {
      ...
      //console.log(this.projectSelected)
      this.http
        .put<any>(urlApi + "/project/" + project.ID + querystring, project)
        .subscribe(
          (any) => {
            if (any) {
      ...
      this.http.post<any>(urlApi + "/project", project).subscribe(
        (any) => {
          if (any) {
            console.log(this.projectSuccessAlert);
            this.openSuccessAlert(
              this.projectSuccessAlert,
              project.ProjectName
            );
  ...
  deleteProject(project: Project) {
    ...
    this.panelExpanded = true;
    if (project.ID != 0) {
      this.http.delete<any>(urlApi + "/project/" + project.ID).subscribe(
        (any) => {
          if (any) {
            this.getProjects();
            this.sheets = [];
  ...
  exportTreeNodesRecur(activeNode: any, treePos: any) {
    this.exportElements =
      this.exportElements + treePos + ":" + activeNode.data.name + ";";
    treePos = treePos + 1;
  ...
  exportTreeNodes(tree: any) {
    ...
    setTimeout(() => {
      let fileName: any =
        this.projectSelected.ProjectName + "-" + activeNode.data.name;
      window.open(
        urlApi +
          "/export-tree?n=" +
          encodeURIComponent(fileName) +
          "&d=" +
          encodeURIComponent(this.exportElements),
        "_blank"
      );
    }, 500);
  }
  ...
  generateGraph(tree: any) {
    ...
    let baseRoot = _.find(this.paperView.graph.getElements(), (item) => {
      return item.attributes.tree_id == activeNode.data.id;
    });
    setTimeout(() => {
      this.paperView.adjustGraphContent();
    }, 800);
    ...
    if (this.paperView.graph.getElements().length <= 0) {
      ...
      if (baseRoot) {
        // only generate/update when tree node exists on sheet
  ...
  generateGraphRecur(activeNode: any, parentNew: any) {
    // generate sheet graph recursive from tree

    let positionType = "position";
    if (activeNode.children.length > 0) {
      let unitX = -1;
      activeNode.children.forEach((child) => {
        if (child.data.isfunctionalrel == true) {
        } else {
          let graphNode = _.find(this.paperView.graph.getElements(), (item) => {
            return item.attributes.tree_id == child.data.id;
          });
  ...
  openAddTreeNodeTemplate(event, inputFormTemplate, tree: any) {
    //  open modal for tree node
    this.tree = tree;
    this.nodeName = "";
    if (!this.projectSelected.ID) {
      alert("No project selected!");
  ...
  openSuccessAlert(projectSuccessTemplate: any, projectName: any) {
    //  open modal for tree node
  ...
  onUpdateTree($event, tree) {
    // save project when tree is updated
    ...
    if (!this.isProjectFisrtTreeUpdate) {
      ...
      this.http
        .put<any>(
          urlApi + "/project/" + this.projectSelected.ID,
          this.projectSelected
        )
        .subscribe(
          (any) => {
  ...
  onNodeFocus($event, tree) {
    // set data when tree node is focused
    ...
    if (
      (nodeCurrent.data.is_displacement &&
        nodeCurrent.data.is_displacement == true) ||
      (nodeCurrent.data.isfunctionalrel &&
        nodeCurrent.data.isfunctionalrel == true)
    ) {
      this.positionCurrent = new Position();
    } else {
  ...
  saveNamePosition(position: any, tree: any) {
    // when position name change in details update name on graph
    var treeNode = this.treeOrg.treeModel.getNodeBy(
      (nodeIn) => nodeIn.data.id == position.ID
    );

    if (treeNode && treeNode.data && treeNode.data.name)
      treeNode.data.name = this.setAandTWhenNeeded(position);
  ...
  changeAdvisingAuthorityPosition(position: any, tree: any) {
    // when position Is Advisor? change in details update on graph
  ...
  savePositionDisplacement(position: any, tree: any) {
    // save displacements
    ...
    if (position.SpacesToSupervisor < node.data.displacement_num) {
      let displacementNode = this.treeOrg.treeModel.getNodeBy((item) => {
        return item.data.displaced_node_id == node.data.id;
      });
      if (displacementNode) {
  ...
  savePosition(position: any, tree: any) {
    // save position (node)
    let treeNodeCurrent;
    if (position && position.ID)
      treeNodeCurrent = this.treeOrg.treeModel.getNodeById(position.ID);
    if (treeNodeCurrent) {
      treeNodeCurrent.data.position = this.positionCurrent;
      if (this.treeNodeCurrent.data.attachments) {
        treeNodeCurrent.data.attachment = this.treeNodeCurrent.data.attachments;
        treeNodeCurrent.data.attachments =
          this.treeNodeCurrent.data.attachments;
      }
      if (this.treeNodeCurrent.data.functionalrels)
        treeNodeCurrent.data.functionalrels =
          this.treeNodeCurrent.data.functionalrels;
    }
  ...
  saveConfig(key: any, value: any) {
    // save config values
    this.http
      .post<any>(urlApi + "/config/" + key, {
        key: key,
        value: value,
      })
      .subscribe(
  ...
  refreshProjectSelectedSheets(cb) {
    // refresh selected project  sheets
    let i = 0;
  ...
  checkBeforeDeleteTreeNodes(tree: any, cb) {
    // check before delete node
  ...
  openConfirmDeleteTreeNodes(
    event: any,
    confirmDeleteTreeNodeTemplate: any,
    tree: any
  ) {
    ...
    this.checkBeforeDeleteTreeNodes(tree, () => {
      if (!this.modalService.hasOpenModals()) {
        //This check is made because the event triggers once per nodeToDelete, so, we open the modal only once
        this.modalWindow = this.modalService.open(
          confirmDeleteTreeNodeTemplate,
          {
            ariaLabelledBy: "modal-basic-title",
            size: "sm",
            scrollable: false,
  ...
  delFunctionalRelsOtherNodes(tree: any, node: any) {
    // ex. del 's5' tree node, find all s5 functional rels on other nodes
  ...
  checkAndUpdateTreeNodeAddedAllSheets(
    parent: any,
    newNodeName: any,
    newNodeId: any
  ) {
    //executed only when node is not root
    this.projectSheets.forEach((sheet) => {
      if (sheet.Data != "") {
  ...
  addNode(tree: any, nodeName: any, nodePositionCode: any) {
    // add tree node
    console.log("addNode");
    ...
    if (tree.treeModel.nodes.length > 0) {
      ...
      nodePosition.PositionInmediateSuperior = this.removeAandTfromName(
        parent.data.name
      );
  ...
  findNonSelectedPreviousSibling(tree) {
    ...
    tree.treeModel.activeNodes.forEach((node) => {
      ...
      let isAmongSelected = this.siblingIsAmongSelectedNodes(
        tree,
        previousSibling
      );
      if (!isAmongSelected) {
        ...
        parentNode = node.realParent
          ...
          : node.treeModel.virtualRoot;
        console.log(
          "Non selected previous sibling is: " + previousSibling.data.name
        );
  ...
  treeNodeOneLevelUp(tree: any) {
    ...
    let parentNode: any = node.realParent
      ? node.realParent
      : node.treeModel.virtualRoot;
    ...
    let grandParentNode: any = parentNode.realParent
      ? parentNode.realParent
      : parentNode.treeModel.virtualRoot;
  ...
  circleCreateNode(event: any) {
    if (event.cardinal == "NORTH" || event.cardinal == "SOUTH") {
      this.paperView.addGraphNode(
        this.paperView.nodeGraphCurrent,
        event.left,
        event.top,
        "Child",
        "",
      ...
      if (event.cardinal == "WEST") siblingSide = "left";
      this.paperView.addGraphNodeSibling(
        this.paperView.nodeGraphCurrent,
        event.left,
        event.top,
        "Child",
  ...
  addNodeDisplacement(tree: any) {
    ...
    this.createNodeDisplacement(
      ...
      () => {
        ...
        displacementNode = tree.treeModel.getNodeBy(
          (nodeIn) => nodeIn.data.displacement_id == displacementId
        );
  ...
  removeNodeDisplacement(tree: any) {
    // delete displacement node
    if (!tree.treeModel.getActiveNode()) {
      alert("No active or selected Node!");
      return;
    }
    ...
    if (
      ...
      node.data.children.forEach((child) => {
        ...
        if (child.data.displacement_num) {
          ...
          if (child.data.displacement_num < 0) {
            child.data.displacement_num = 0;
          }
        }
  ...
  addNodeSibling(tree: any, nodeName: any, nodePositionCode: any) {
    // add  tree node sibling
    if (nodeName == undefined || nodeName == "") {
    ...
    if (tree.treeModel.nodes.length > 0) {
      ...
      setTimeout(() => {
        tree.treeModel
          .getNodeBy((node) => node.data.name == nodeName)
          .setActiveAndVisible();
      }, 300);
    } else {
      //is root
  ...
  deleteAditionalShape(item: any, tree) {
    _.remove(
      this.paperView.shapeProperties.privacy.contactsAditional,
      (contact) => {
        return contact === item;
      }
    );
  ...
  deleteExceptionShape(item: any, tree) {
    _.remove(
      this.paperView.shapeProperties.privacy.contactsException,
      (contact) => {
        return contact === item;
      }
    );
  ...
  onNavChangeNav3(changeEvent: NgbNavChangeEvent) {
    ...
    if (changeEvent.nextId === 3) {
      //  postition employee
      if (
        this.positionCurrent.PositionPurpose == null ||
  ...
  gElements: any = [];
  showNodesByLevel(nodeGraphLevelSelected: any) {
    // show graph nodes by level
  ...
  openConfigDefaultShapeModal(event, inputFormTemplate) {
    ...
    if (this.paperView.shapeProperties.privacy.contactsAditional) {
      this.paperView.shapeProperties.privacy.contactsAditional.forEach(
        (item) => {
          this.contactsAditionalHash[item.name] = true;
        }
      );
    ...
    if (this.paperView.shapeProperties.privacy.contactsException) {
      this.paperView.shapeProperties.privacy.contactsException.forEach(
        (item) => {
          this.contactsExceptionHash[item.name] = true;
        }
      );
  ...
  saveActiveSheetShapesDefaults() {
    this.saveConfig(
      "sheet" + this.sheetSelected.ID,
      JSON.stringify(this.paperView.shapeProperties)
    );
  }

  getActiveSheetShapesDefaults() {
    // get defaults foe active sheet
    if (!this.sheetSelected || !this.sheetSelected.ID) {
      return;
    }
    this.http
      .get<any>(urlApi + "/config/sheet" + this.sheetSelected.ID)
      .subscribe(
        (any) => {
          if (any && any[0]) {
            if (any[0].Value != "") {
              ...
              //init properties if new or not exists
  ...
  openTranslateAllModal(event, inputFormTemplate) {
    // translate graph
    if (!this.sheetSelected.ID) {
      alert("No Sheet Selected!");
      return;
  ...
  translateAll(dir: any, units: any) {
    // translate all nodes on sheet n units
    if (!units) {
      units = 0;
    }
    if (dir == "up") {
      this.paperView.graph.translate(0, Math.trunc(units * -1));
    }
    if (dir == "left") {
      this.paperView.graph.translate(Math.trunc(units * -1), 0);
    }
    if (dir == "right") {
      this.paperView.graph.translate(Math.trunc(units), 0);
    }
    if (dir == "down") {
      this.paperView.graph.translate(0, Math.trunc(units));
    }
  }

  setSheetNewSize(event: any) {
    // on paper size dimentions change
    if (!this.sheetSelected.ID) {
      alert("No Sheet Selected!");
      return;
    }
  ...
  nodeGraphNameChange(event: any) {
    // when node name  on graph is changed, update on tree node
    ...
    if (treeNode) {
      if (
        treeNode.data.name.includes("(a)") &&
        treeNode.data.name.includes("(t)")
      )
        treeNode.data.name = "(a) (t) " + event.name;
      if (
        treeNode.data.name.includes("(a)") &&
        !treeNode.data.name.includes("(t)")
      )
        treeNode.data.name = "(a) " + event.name;
      if (
        !treeNode.data.name.includes("(a)") &&
        treeNode.data.name.includes("(t)")
      )
        treeNode.data.name = "(t) " + event.name;
      treeNode.data.position.PositionName = this.removeAandTfromName(
        event.name
      );
    }
  ...
  graphNodeAdded(event: any) {
    // new node added on graph, check for element tree id to relate both
    let gEl = _.find(this.paperView.graph.getElements(), (gEl) => {
      return gEl.attributes.id == event.attributes.id;
    });
    let treeNode = this.treeOrg.treeModel.getNodeBy((item) => {
    ...
    if (treeNode) {
      ...
      // new sheet node does not exist on tree, use father id, to create new tree node and associate both
      let gElParent = _.find(this.paperView.graph.getElements(), (gEl) => {
        return gEl.attributes.id == event.attributes.org_parent_id;
      });
      ...
      if (this.treeOrg.treeModel.nodes.length > 0) {
        if (treeNodeParent) {
          ...
          setTimeout(() => {
            ...
            if (newTreeNode) {
              if (gEl.attributes.is_advisor)
                newTreeNode.data.name = "(a) " + newTreeNode.data.name;
      ...
      if (
        this.sheetSelected &&
        this.sheetSelected.ID &&
        this.sheetSelected.ID != 0
      ) {
        setTimeout(() => {
          this.saveSheet(this.sheetSelected);
        }, 800);
  ...
  addAttachment(attachment: any) {
    ...
    this.treeNodeCurrent.data.attachments.sort((a, b) =>
      a.name.slice(11).toLowerCase() > b.name.slice(11).toLowerCase()
        ? 1
        : b.name.slice(11).toLowerCase() > a.name.slice(11).toLowerCase()
        ? -1
        : 0
    );
  ...
  openConfirmDeleteAttachment(
    event: any,
    attachment: any,
    confirmDeleteAttachmentTemplate: any
  ) {
    // modal confirm delete node
    ...
    if (!this.modalService.hasOpenModals()) {
      //This check is made because the event triggers once per nodeToDelete, so, we open the modal only once
      this.modalWindow = this.modalService.open(
        confirmDeleteAttachmentTemplate,
        {
          ariaLabelledBy: "modal-basic-title",
          size: "sm",
          scrollable: false,
  ...
  openSetImendiateSuperior(event, template) {
    // open set inmediate superior modal
    event.preventDefault();
  ...
  onNodeSelectSetImendiateSuperior($event, tree: any, modal: any) {
    //set inmediate superior modal
    let node = tree.treeModel.getFocusedNode();
    //console.log("Seleccionado " + node.data.name + " como el nodo padre");

    let nodeCurrent = this.treeOrg.treeModel.getNodeBy((item) => {
      return this.treeNodeCurrent.data.id == item.data.id;
    });
    ...
    if (node.data.name == nodeCurrent.data.name) {
      alert(
        "Position " +
          nodeCurrent.data.name +
          " can't be immediate superior for himself"
      );
      modal.close();
      return;
    }

    let parentNode: any = nodeCurrent.realParent
      ? nodeCurrent.realParent
      : nodeCurrent.treeModel.virtualRoot;
    if (nodeCurrent) {
      ...
      this.positionCurrent.PositionInmediateSuperior = this.removeAandTfromName(
        node.data.name
      );
      this.treeOrg.treeModel.update();
      this.onUpdateTree(null, this.treeOrg);
  ...
  onNodeSelectFunctionRel($event, tree: any, modal: any) {
    //set functional rel
  ...
  openAddFunctionalRel(event, template) {
    // open functional rel modal
  ...
  openConfirmDeleteFuncRel(
    event: any,
    functionalrel: any,
    confirmDeleteFuncRelTemplate: any
  ) {
    // modal confirm delete node
  ...
  deleteFunctionalRel() {
    ...
    let treeNodeParentWithFunctionalRelToDelete =
      this.treeOrg.treeModel.getNodeById(
        this.parentWithFunctionalRelToDelete.id
      );
    ...
    treeNodeParentWithFunctionalRelToDelete.data.children.forEach((child) => {
      if (child.functionalRelTargetId == this.treeNodeCurrent.data.id)
        fToDelete = child;
    });
  ...
  deleteFunctionalRelById(functionalRelTarget: any, tree: any) {
    let nodeWithFunctionRel = tree.treeModel.getNodeById(
      functionalRelTarget.id
    );
  ...
  createNodeFunctionalRel(
    tree: any,
    parentNode: any,
    functionralRelId: any,
    nodeWithFunctionalRel: any,
    cb
  ) {
    let nodePosition: Position = new Position();
  ...
  getSheetMinLevelNode(cells: any) {
    //get root node on  sheet
  ...
  updateAllSheetsFromTreeNode() {
    ...
    this.projectSheets.forEach((sheet) => {
      if (sheet.Data != "") {
        ...
        if (rootSheetNode) {
          let treeNodeRootForSheet = this.treeOrg.treeModel.getNodeBy(
            (node) => node.data.id == rootSheetNode.tree_id
          );
          if (!treeNodeRootForSheet) {
            console.log(
              "updateAllSheetsFromTreeNode",
              "!treeNodeRootForSheet",
              sheet.SheetName
            );
          }
          if (treeNodeRootForSheet) {
            let treeNodeRootForSheetDataName = this.removeAandTfromName(
              treeNodeRootForSheet.data.name
            );
            let newCell = this.paperView.memberDef(
              null,
              350,
            ...
            let cCellAttributes = this.getCurrentCellAttributes(
              cells,
              treeNodeRootForSheet.data.id
            );
  ...
  updateNodesTreesParent(activeNode: any) {
    if (activeNode.children && activeNode.children.length > 0) {
      activeNode.children.forEach((child) => {
        child.position.PositionInmediateSuperior = this.removeAandTfromName(
          activeNode.name
        );
        this.treeOrg.treeModel.update();
    ...
    if (activeNode.children.length > 0) {
      ...
      activeNode.children.forEach((child) => {
        if (child.data.isfunctionalrel == true) {
          ...
          newCell.attributes.tree_id = child.data.id;
          cells.cells.push(newCell.attributes);
          let newLink = this.paperView.getLinkDef(
            parentNew,
            newCell,
            child.data.is_displacement
          );
          cells.cells.push(newLink.attributes);
          sheet.Data = JSON.stringify(cells);
          unitX = unitX + 1;
  ...
  onSearchContacts(n) {
    ...
    this.contactsSearch = this.contactsSearch.filter((item) => {
      return item.firstName.toLowerCase().indexOf(n) > -1;
    });
  }
